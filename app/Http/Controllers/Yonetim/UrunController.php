...
class UrunController extends Controller
{
    ...
    public function form($id = 0)
    {
        ...
        if ($id > 0) {
            $entry = Urun::find($id);
            //pluck: Bir tablodan sadece belirli bir kolon değerini al burada sadece id değerini aldım
    ...
    public function kaydet($id = 0)
    {
        ...
        if ($id > 0) {
            ...
            $entry->update($data);
            //Laravel içerisinde ilişkili tablolarda update,create komutu dizi ile çalışır
            //save metodu da aynı mantıkla çalışır, model sınıfını ister yani dizi olarak değilde object türünde ister
            $entry->detay()->update($data_detay);
            //sync: Senkron hale getir gönderilen değerler ile beraber senkron hale getirir
            $entry->kategoriler()->sync($kategoriler);
        } else {
            $entry = Urun::create($data);
            $entry->detay()->create($data_detay);
            //Dizi şeklinde aldığı kategori bilgisini tek tek kategori_uru  tablosuna ekler
        ...
        if (request()->hasFile('urun_resmi')) {
            ...
            /*Ben dosya yüklerken dosyam bellekte geçici olarak tutulur
            isValid metodu geçici alana dosyanın düzgün bir şekilde yükleyip yüklemediği kontrol eder*/
            if ($urun_resmi->isValid()) {
                //Ürün resmini belirlenen klasöre belirlenen dosya adı ile  kaydet
